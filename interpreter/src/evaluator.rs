...
fn eval_app(cur_expr: LExpr,
  ...
  match cur_expr {
    ...
    LFun(fun) => {
      return match fun {
        LFPlus() => {
          let arg1 = eval(app_spine.pop().unwrap(), env);
          let arg2 = eval(app_spine.pop().unwrap(), env);
